...
void bst_replace_by_rightmost(bst_node_t *target, bst_node_t **tree) {
  if (!((*tree)->right)) { //pokud nemá pravý podstrom
    bst_node_t *tmp = (*tree);
    printf("hejko");
  ...
  if(!((*tree)->right->right)) {  //pokud neexistuje podstrom pravého podstromu
...
void bst_delete(bst_node_t **tree, char key) {
  ...
  if (key == (*tree)->key) {
    ...
    //jeden podstrom
    if (!((*tree)->left) && (*tree)->right) { //má pravý podstrom
      bst_node_t *tmp = (*tree);
      (*tree) = ((*tree)->right);
      free(tmp);
      return;
    ...
    if ((*tree)->left && !((*tree)->right)) { //má levý podstrom
      bst_node_t *tmp = (*tree);
      (*tree) = ((*tree)->left);
      free(tmp);
      return;
